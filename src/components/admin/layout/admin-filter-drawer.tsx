...
export function AdminFilterDrawer({
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent
        side="right"
        className="w-full sm:max-w-md p-0 flex flex-col bg-white border-l border-slate-200 shadow-xl"
      >
        {/* Cabecera del Drawer */}
        <SheetHeader className="p-5 border-b border-slate-100 text-left space-y-1">
        ...
        <SheetFooter className="p-4 border-t border-slate-100 bg-slate-50/50 flex-row justify-end gap-2 sm:space-x-0">
          {onReset && (
            <AdminButton
              type="button"
              variant="outline"
