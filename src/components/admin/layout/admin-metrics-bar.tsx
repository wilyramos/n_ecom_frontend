...
export interface MetricItem {
  ...
  onClick?: () => void; // <-- Propiedad añadida para filtrar
}
      ...
      {isOpen && (
        <div className="grid grid-cols-2 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-slate-200 bg-white">
          {metrics.map((item, idx) => {
            ...
            // Si tiene onClick lo convertimos en un botón interactivo
            ...
            return (
              <div
                ...
                className={cn(
                  "px-3.5 py-2 flex items-center justify-between gap-3 transition-colors",
                  isClickable ? "cursor-pointer hover:bg-slate-100 active:bg-slate-200" : "hover:bg-slate-50/50"
                )}
